<template>
    <div>
        ...
        <van-form validate-first style="margin-top: 200px;">
            <van-field v-model="shop.oldPwd" placeholder="请输入旧密码"
            ...
            <van-field v-model="shop.newPwd" placeholder="请输入3位数字的新密码"
                style="margin: 10px auto;width: 90%;background-color: rgba(152, 230, 240, 0.6);" left-icon="manager-o"
                :rules="[{ pattern, message: '请输入正确格式的密码' }]" label="新密码" />
            <van-field v-model="shop.AgainNewPwd" placeholder="请再次输入新密码"
                style="margin: 10px auto;width: 90%;background-color: rgba(152, 230, 240, 0.6);" left-icon="manager-o"
    ...
    methods: {
        submit() {
            Dialog.confirm({
                ...
                if (this.shop.newPwd === this.shop.AgainNewPwd && this.shop.oldPwd !== '') {
                    axios.put('identity/updatePwd/' + this.shopId + '/' + this.shop.oldPwd + '/' + this.shop.newPwd).then(res => {
                        if (res.data.flag) {
                            Dialog.alert({
                ...
                } else if (this.shop.newPwd !== this.shop.AgainNewPwd) {
