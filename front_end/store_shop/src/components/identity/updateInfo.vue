<template>
    <div>
        ...
        <van-form validate-first>
            <van-field v-model="shop.name" placeholder="请输入商铺名"
            ...
            <van-field v-model="shop.tel" placeholder="请输入电话号码"
                style="margin: 10px auto;width: 90%;background-color: rgba(152, 230, 240, 0.6);" left-icon="manager-o"
...
export default {
    ...
    methods: {
        updateImg(file) {
            this.shop.img = file.file.name
            utils.imgSuccess(file.file)
        },
        submit() {
            if (!this.isNull) {
                axios.put('/identity/updateShop', this.shop).then(res => {
                    if (res.data.flag) {
                        Toast('资料修改成功')
                        this.getInfo()
                    }
        ...
        // 检验是否重复昵称
        checkName() {
            axios.get('/identity/getInfo/' + this.shopId + '/' + this.shop.name).then(res => {
                if (!res.data.flag) {
                    Dialog.alert({
