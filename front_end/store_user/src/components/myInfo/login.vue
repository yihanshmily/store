<template>
    <div>
        <!-- 商标 -->
        <div style="width: 100%;margin: 40px 90px;font-size:50px;
            font-family: '华文彩云';color: darkblue;letter-spacing: 100px;">绫瑶</div>
        <!-- 头像 -->
        <div style="margin: 60px 30% 50px 30%;">
            <van-image v-if="userImg    " round width="10rem" height="10rem" :src="utils.getImage(userImg)"/>
            <van-image v-else round width="10rem" height="10rem" :src="utils.getImage('default.png')"/>
        </div>
        <!-- 输入框 -->
        <van-form validate-first>
            <van-field v-model="login.name" placeholder="请输入用户名或用户账号" style="margin: 10px auto;width: 70%;background-color: rgba(152, 230, 240, 0.6);" 
                left-icon="manager-o" :rules="[{ pattern, message: '请输入正确的用户名或用户账号' }]" @blur="getImg"/>
        ...
        <div style="display: flex;">
            ...
            <div style="width: 50%;margin-left: 40px;font-size: 18px;">没有账号？
...
export default {
    ...
    methods: {
        ...
        getImg(){
            axios.get('/identity/getImg/user/'+this.login.name).then(res=>{
                if(res.data.flag){
        ...
        submit(){
            if (/^[A-Za-z0-9_\-\u4e00-\u9fa5]+$/.test(this.login.name && /^(?=.*\d).{3}$/.test(this.login.pwd))){
                axios.post('/login/user',this.login).then(res=>{
                    if(res.data.flag){
                        Toast.success('登录成功')
